...
function lecturaEjecutivaMes(monthData) {
    ...
    return `<strong>${mesNombrePropioText(monthData.label)}:</strong> el universo mensual se compone de ${fmt(monthData.acreditado)} personas con acreditación vigente y ${fmt(sinAcreditacionVigente)} personas sin acreditación vigente. Dentro de este último grupo, ${fmt(monthData.noVigente)} corresponden a personas no vigentes y ${fmt(monthData.noAcreditado)} a personas sin acreditación previa.`;
}
    ...
    if (!dashboardData.hasComunaData) {
        ...
        contenedor.innerHTML = `
            <div class="modal-grid kpi-section modal-kpi-grid">
                ...
                <div class="kpi-card rojo"><div class="kpi-label">N° de personas sin acreditación previa</div><div class="kpi-value">${fmt(regionData.NoAcreditado)}</div><div class="kpi-percent">${porcentajeRatioEs(regionData.NoAcreditado, regionData.Total)} del total regional.</div></div>
...
function renderDashboard() {
    ...
    document.getElementById("content").innerHTML = `
        ...
        <div class="executive-band">
            <div class="panel executive-main"><div class="executive-title">Lectura ejecutiva</div><div class="executive-text">${lecturaEjecutivaMes(monthData)}</div><div class="definitions-compact"><strong>Con acreditación vigente:</strong> persona con acreditación vigente en el mes seleccionado. <strong>Sin acreditación vigente:</strong> personas que actualmente no cuentan con acreditación activa. <strong>No vigente:</strong> persona que tuvo acreditación previa, pero no cuenta con vigencia actual. <strong>Sin acreditación previa:</strong> persona que no registra acreditación anterior.</div></div>
            <div class="panel executive-side"><div class="executive-title">Contexto</div><div class="context-list"><div><span>Mes visualizado</span><strong>${mesNombrePropioText(monthData.label)}</strong></div><div><span>Fuente</span><strong>Sistema de Información Social en Emergencias</strong></div><div><span>N° de registros</span><strong>${fmt(monthData.total)}</strong></div><div><span>Comparación</span><strong>${prevMonthData ? mesNombrePropioText(prevMonthData.label) : "Sin mes previo"}</strong></div></div></div>
        </div>
        ...
        <div class="kpi-section">
            ...
            <div class="kpi-card ambar"><div class="kpi-label">N° de personas no vigentes</div><div class="kpi-value">${fmt(monthData.noVigente)}</div><div class="kpi-percent">${porcentajeRatioEs(monthData.noVigente, monthData.total)} del total de registros de ${mesNombrePropioText(monthData.label)}</div><div class="kpi-description">Subgrupo de personas sin acreditación vigente.</div><div class="kpi-delta ${deltaNoVigente.className}">${deltaNoVigente.label}</div></div>
            <div class="kpi-card rojo"><div class="kpi-label">N° de personas sin acreditación previa</div><div class="kpi-value">${fmt(monthData.noAcreditado)}</div><div class="kpi-percent">${porcentajeRatioEs(monthData.noAcreditado, monthData.total)} del total de registros de ${mesNombrePropioText(monthData.label)}</div><div class="kpi-description">Subgrupo de personas sin acreditación vigente.</div><div class="kpi-delta ${deltaNoAcreditado.className}">${deltaNoAcreditado.label}</div></div>
        </div>
        <div class="charts-section">
            <div class="chart-card"><div class="section-title">Evolución mensual según acreditación vigente</div><div class="section-note">Permite comparar la evolución de personas con acreditación vigente, no vigentes y sin acreditación previa entre meses informados.</div><div class="chart-canvas-wrap"><canvas id="trendChart"></canvas></div></div>
            <div class="chart-card"><div class="section-title">Distribución por acreditación vigente</div><div class="section-note">Resumen compacto del mes seleccionado, con colores semánticos consistentes en todo el tablero.</div><div class="chart-canvas-wrap compact"><canvas id="estadoChart"></canvas></div></div>
        </div>

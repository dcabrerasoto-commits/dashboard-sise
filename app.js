...
function renderDashboard() {
    ...
    document.getElementById("content").innerHTML = `
        ...
        <div class="executive-band">
            ...
            <div class="panel executive-side"><div class="executive-title">Contexto</div><div class="context-list"><div><span>Mes visualizado</span><strong>${mesNombrePropioText(monthData.label)}</strong></div><div><span>Fuente</span><strong>Sistema de Información Social en Emergencias</strong></div><div><span>N° de registros del mes</span><strong>${fmt(monthData.total)} registros de ${mesNombrePropioText(monthData.label)}</strong></div><div><span>Comparación</span><strong>${prevMonthData ? `${mesNombrePropioText(prevMonthData.label)} (${fmt(prevMonthData.total)} registros)` : "Sin mes previo"}</strong></div><div><span>Registros del mes anterior</span><strong>${prevMonthData ? fmt(prevMonthData.total) : "Sin mes previo"}</strong></div><div><span>Variación del universo mensual</span><strong>${prevMonthData ? `${deltaUniverso >= 0 ? "+" : ""}${fmt(deltaUniverso)} registros` : "Sin mes previo"}</strong></div></div><div class="context-note">Las variaciones de los KPIs comparan el número de personas por estado respecto del mes anterior. No representan necesariamente traspasos directos entre estados, porque el universo mensual puede variar.</div></div>
        </div>
